...
extends DependenciesSyntaxChecker {
    ...
    private DraftV3DependenciesSyntaxChecker() {
        NodeType[] arrnodeType = new NodeType[]{NodeType.ARRAY, NodeType.STRING};
        super(arrnodeType);
    }
    ...
    public void checkDependency(ProcessingReport processingReport, MessageBundle messageBundle, String string, SchemaTree schemaTree) throws ProcessingException {
        ...
        if (n2 == 0) {
            processingReport.warn(this.newMsg(schemaTree, messageBundle, "common.array.empty").put("property", string));
            return;
        }
        ...
        if (!bl) {
            processingReport.warn(this.newMsg(schemaTree, messageBundle, "common.array.duplicateElements").put("property", string));
        }
    }
}
