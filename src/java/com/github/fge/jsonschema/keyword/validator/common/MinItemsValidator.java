...
extends PositiveIntegerValidator {
    public MinItemsValidator(JsonNode jsonNode) {
        super("minItems", jsonNode);
    }
    ...
    public void validate(Processor<FullData, FullData> processor, ProcessingReport processingReport, MessageBundle messageBundle, FullData fullData) throws ProcessingException {
        ...
        if (n2 < this.intValue) {
            processingReport.error(this.newMsg(fullData, messageBundle, "err.common.minItems.arrayTooShort").putArgument(this.keyword, this.intValue).putArgument("found", n2));
        }
    }
}
