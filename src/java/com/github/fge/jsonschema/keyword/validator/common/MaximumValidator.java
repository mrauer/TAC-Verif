...
extends NumericValidator {
    ...
    public void validateDecimal(ProcessingReport processingReport, MessageBundle messageBundle, FullData fullData) throws ProcessingException {
        ...
        if (n2 > 0) {
            processingReport.error(this.newMsg(fullData, messageBundle, "err.common.maximum.tooLarge").putArgument(this.keyword, this.number).putArgument("found", jsonNode));
            return;
        }
        if (!this.exclusive) {
            return;
        }
        processingReport.error(this.newMsg(fullData, messageBundle, "err.common.maximum.notExclusive").putArgument(this.keyword, this.number).put("exclusiveMaximum", (JsonNode)BooleanNode.TRUE));
    }
